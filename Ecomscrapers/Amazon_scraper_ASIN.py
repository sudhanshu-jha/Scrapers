# Program to get price from Amzon.in using ASIN number
...
def AmzonParser(url):
    ...
    while True:
        ...
        try:
            ...
            SALE_PRICE = ' '.join(''.join(RAW_SALE_PRICE).split()).strip(
            ) if RAW_SALE_PRICE else None
            CATEGORY = ' > '.join(
...
def ReadAsin():
    AsinList = ['B073Q47TSH', 'B06Y3GYZKW']
    extracted_data = []
    for i in AsinList:
