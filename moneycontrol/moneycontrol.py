    ...
    if successfulRequest(r.status_code):
        ...
        if linkSource:
            ...
            if successfulRequest(r.status_code):
                ...
                companyDiv = parentDiv.find_all(
                    'div', attrs={'class': 'w252 FL'})

                # The DIV that contains registrar information
                registrarDiv = parentDiv.find_all(
                    'div', attrs={'class': 'w230 FL PR25 PL25'})
...
def main():
    ...
    # List of dictionaries, each of which contains the company and registrar
    # contact details for a given stock
    detailsList = []

    # List of ISIN numbers for which the program couldn't find details
    # automatically, must look these up manually
    manualList = []
